...
/**
 * // In the case of the natural logarithm, there is an easy way to reduce the
 * range. Real numbers are almost universally represented in terms of a mantissa
 * and an exponent: x=m*2p, where p is an integer and m is between 1 and 2. Thus
 * log(x) = log(m)+p*log(2). The latter term, p*log(2), is just a multiplication
 * by a known constant. The problem thus reduces to finding the logarithm of a
 * number between 1 and 2 (or between 1/2 and 1). A further range reduction can
 * be made by using the fact that √2 is logarithmically in the middle of [1,2).
 * Thus all that is needed is a way to calculate the logarithm of a number
 * between 1 and √2. This is typically done with a rational polynomial. The
 * ratio of a second order polynomial polynomial to a third order works quite
 * nicely for this.
 *
 */
...
// Pole errors occur when an operation has a result that is asymptotically
// infinite, such as divisions by zero, or log(0.0) . Pole error: division by
// zero, or some other asymptotically infinite result (from finite arguments).
...
START_TEST(log_negative) { ck_assert_ldouble_nan(s21_log(-314)); }
END_TEST

START_TEST(log_test_5_zero) { ck_assert_ldouble_infinite(s21_log(0)); }
END_TEST
...
Suite *suite_s21_log(void) {
    ...
    tcase_add_test(tc, log_negative);
